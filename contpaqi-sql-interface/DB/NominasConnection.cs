...
{
    public class NominasConnection: SystemConnection
    {
        public NominasConnection(CoreDB coreDB) : base(coreDB, "nomGenerales") { }

        public override PanelEntry[] GetDatabasesForAttach()
        {
            string query = $@"
                select NombreEmpresa, RutaEmpresa from {indexName}..NOM10000 WHERE RutaEmpresa NOT IN (
                    SELECT
